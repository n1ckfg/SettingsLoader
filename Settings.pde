  ...
  Settings(String _s) {
    try {
      ...
      for (int i=0;i<settings.data.length;i++) {
        if (settings.data[i].equals("drawCircle")) drawCircle = setBoolean(settings.data[i+1]);
        if (settings.data[i].equals("xPos")) p.x = setFloat(settings.data[i+1]);
        if (settings.data[i].equals("yPos")) p.y = setFloat(settings.data[i+1]);
        if (settings.data[i].equals("fillColor")) fillColor = setColor(settings.data[i+1]);
        if (settings.data[i].equals("strokeColor")) strokeColor = setColor(settings.data[i+1]);
        if (settings.data[i].equals("strokeWeight")) strokeWeightNum = setInt(settings.data[i+1]);
      }
    } 
    catch(Exception e) {
